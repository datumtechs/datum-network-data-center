...
CREATE TABLE org_info (
    ...
    node_id VARCHAR(128) NOT NULL COMMENT '组织节点ID',
...
CREATE TABLE task (
    ...
    owner_party_id VARCHAR(128) NOT NULL COMMENT '任务参与方在本次任务中的唯一识别ID',
...
CREATE TABLE task_algo_provider (
    ...
    identity_id VARCHAR(128) NOT NULL COMMENT '算法提供者组织身份ID',
    party_id VARCHAR(128) NOT NULL COMMENT '任务参与方在本次任务中的唯一识别ID',
    PRIMARY KEY (task_ID)
) comment '任务算法提供者';
...
CREATE TABLE task_result_consumer (
    ...
    consumer_party_id VARCHAR(128) NOT NULL COMMENT '任务参与方在本次任务中的唯一识别ID',
    producer_identity_id VARCHAR(128) NOT NULL COMMENT '结果产生者的组织身份ID',
    producer_party_id VARCHAR(128) NOT NULL COMMENT '任务参与方在本次任务中的唯一识别ID',
...
CREATE TABLE power_change_history (
    id VARCHAR(128) NOT NULL comment 'power_server.ID,hash',
...
begin
    ...
    values (NEW.id, NEW.memory, NEW.core, NEW.bandwidth, 'increased', CURRENT_DATE());
end
$$
DELIMITER ;
...
begin
    ...
    ELSEIF OLD.status='revoke' AND NEW.status='release' THEN
        ...
        values (OLD.id, OLD.size, 'release', 'increased', CURRENT_DATE());
    END IF;
end
$$
DELIMITER ;
